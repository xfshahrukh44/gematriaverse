...
function Set_Dates(impVal, impNum) {
	...
	DurationTable()
	if (impVal == 2) {
		document.getElementById("AddSubLink").innerHTML = '<a href="javascript:DropCells()">Add/Subtract</a>'
		dText = ""
		opt_AddSub = "None"
		document.getElementById("AddSubBoxes").innerHTML = dText
	} else if (opt_AddSub !== "None") {
		SetDatesAlt()
	}
}

function SetDatesAlt() {
	PopDates(false)
	ChangeDate2()
	PopDates()
	Pop_DateStrings()
...
function Set_Options() {
	...
	*/
	...
	if (opt_AddSub !== "None") {SetDatesAlt()}
}
...
function DropCells(impOpen = true) {
	var dest = document.getElementById("AddSubBoxes")
	var dText

	if (impOpen == true) {
		opt_AddSub = "Add"
		document.getElementById("AddSubLink").innerHTML = '<a href="javascript:DropCells(false)">Close</a>'
		dText = '<center><table><tr><td><input type="checkbox" value="AddBox" id="AddChoice" onclick="ClickAddSub(true)">Add</input><BR>'
		dText += '<input type="checkbox" value="SubtractBox" id="SubtractChoice" onclick="ClickAddSub(false)">Subtract</input>'
		dText += '<td class="u_Entry">Years:<BR><input type="number" max="2020" min="0" class="u_Inp" id="YearAdd" value="0" oninput="SetDatesAlt()"></input></td>'
		dText += '<td class="u_Entry">Months:<BR><input type="number" max="24000" min="0" class="u_Inp" id="MonthAdd" value="0" oninput="SetDatesAlt()"></input></td>'
		dText += '<td class="u_Entry">Weeks:<BR><input type="number" max="100000" min="0" class="u_Inp" id="WeekAdd" value="0" oninput="SetDatesAlt()"></input></td>'
		dText += '<td class="u_Entry">Days:<BR><input type="number" max="700000" min="0" class="u_Inp" id="DayAdd" value="0" oninput="SetDatesAlt()"></input></td></tr></table></center>'
		dest.innerHTML = dText
		if (opt_AddSub == "Add") {document.getElementById("AddChoice").checked = true} else {document.getElementById("SubtractChoice").checked = true}
	} else {
		opt_AddSub = "None"
		document.getElementById("AddSubLink").innerHTML = '<a href="javascript:DropCells()">Add/Subtract</a>'
		dText = ""
...
function ClickAddSub (impType) {
	var aBox, sBox
	aBox = document.getElementById("AddChoice")
	sBox = document.getElementById("SubtractChoice")
...
function PopDates(impOpt = true) {
	var x, y
	var cMax
	if (impOpt == true) {cMax = CountDates} else {cMax = 1}
...
function ChangeDate2() {
	var yBox, mBox, wBox, dBox, tDate, dateAdd
	yBox = Number(document.getElementById("YearAdd").value)
	mBox = Number(document.getElementById("MonthAdd").value)
	wBox = Number(document.getElementById("WeekAdd").value)
	dBox = Number(document.getElementById("DayAdd").value)
	dateAdd = (wBox * 7) + dBox

	if (opt_AddSub == "Add") {
		tDate = new Date(wDate[1].getFullYear() + yBox, wDate[1].getMonth(), wDate[1].getDate())
		tDate = new Date(tDate.getFullYear(), tDate.getMonth() + mBox, tDate.getDate())
		if (wDate[1].getDate() !== tDate.getDate()) {
			tDate = new Date(tDate.getFullYear(), tDate.getMonth(), tDate.getDate() - tDate.getDate())
		}
		tDate = new Date(tDate.getFullYear(), tDate.getMonth(), tDate.getDate() + dateAdd)
		if (oArr.EndInc == true) {tDate = new Date(tDate.getFullYear(), tDate.getMonth(), tDate.getDate() - 1)}
	} else {
		tDate = new Date(wDate[1].getFullYear() - yBox, wDate[1].getMonth(), wDate[1].getDate())
		tDate = new Date(tDate.getFullYear(), tDate.getMonth() - mBox, tDate.getDate())
		if (wDate[1].getDate() !== tDate.getDate()) {
	...
	document.getElementById("Month2").value = tDate.getMonth() + 1
	document.getElementById("Day2").value = tDate.getDate()
	document.getElementById("Year2").value = tDate.getFullYear()
}
...
function Set_Durations() {
	...
	yearDiff = bigDate.getFullYear() - tempDate.getFullYear()
