...
function AddTerm() {
    ...
    xhttp.open("POST", "php/updatematchdbNG.php?phrase=" + qStr + "&table=4&submit=1", true);
    xhttp.send();
}
...
function Open_Shortcuts() {
	window.open("/nextgen/Shortcuts.png", 'Keyboard Shortcuts')
}
...
function Get_Matches() {
	...
	if (ciphersOn.length < 13) {
		...
		if (NumberArray() == true) {
			xhttp.open("GET", "/nextgen/numbermatch.php?search=" + SearchNumbers(), true);
			xhttp.send();
		} else {
			xhttp.open("GET", "/nextgen/getmatch.php?search=" + SearchString(), true);
			xhttp.send();
		}
	} else {
...
function UpdateUserHistory() {
    ...
    xhttp.open("POST", "/usersettings/updatehistory.php?history=" + cString, true);
    xhttp.send();
}
function UpdateOptions() {
    ...
    xhttp.open("POST", "/usersettings/updateoptions.php?options=" + oString, true);
    xhttp.send();
}