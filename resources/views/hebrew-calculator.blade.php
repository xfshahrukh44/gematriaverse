                ...
                <div class="col-lg-12">
                    <div id="ChartSpot">
                        <table>
                            ...
                            <tr></tr>
                            <tr>
                                <td class="chartChar">א</td>
                                <td class="chartChar">ב</td>
                                <td class="chartChar">ג</td>
                                <td class="chartChar">ד</td>
                                <td class="chartChar">ה</td>
                                <td class="chartChar">ו</td>
                                <td class="chartChar">ז</td>
                                <td class="chartChar">ח</td>
                                <td class="chartChar">ט</td>
                                <td class="chartChar">י</td>
                                <td class="chartChar">כ</td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                <td class="chartChar">ל</td>
                                <td class="chartChar">מ</td>
                                <td class="chartChar">נ</td>
                                <td class="chartChar">ס</td>
                                <td class="chartChar">ע</td>
                                <td class="chartChar">פ</td>
                                <td class="chartChar">צ</td>
                                <td class="chartChar">ק</td>
                                <td class="chartChar">ר</td>
                                <td class="chartChar">ש</td>
                                <td class="chartChar">ת</td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                <td id="cipherChartTitle" colspan="11">
{{--                                    <span id="moveCipherUp" class="moveCipher">--}}
{{--                                        <i class="fas fa-chevron-circle-up"></i>--}}
{{--                                    </span>--}}
                                    <font style="color: RGB(255, 227, 93)">Hebrew Standard</font>
{{--                                    <span id="moveCipherDown" class="moveCipher" onclick="javascript:MoveCipher(undefined, 2)">--}}
...
@section('js')
    <script>
        // Hebrew alphabet mapping for Standard (Isopsephy) values
        const hebrewStandardAlphabet = {
            'א': 1, 'ב': 2, 'ג': 3, 'ד': 4, 'ה': 5, 'ו': 6, 'ז': 7, 'ח': 8, 'ט': 9, 'י': 10, 'כ': 20,
            'ל': 30, 'מ': 40, 'נ': 50, 'ס': 60, 'ע': 70, 'פ': 80, 'צ': 90, 'ק': 100, 'ר': 200, 'ש': 300, 'ת': 400
        };

        // Hebrew alphabet mapping for Ordinal values (1 to 22)
        const hebrewOrdinalAlphabet = {
            'א': 1, 'ב': 2, 'ג': 3, 'ד': 4, 'ה': 5, 'ו': 6, 'ז': 7, 'ח': 8, 'ט': 9, 'י': 10, 'כ': 11,
            'ל': 12, 'מ': 13, 'נ': 14, 'ס': 15, 'ע': 16, 'פ': 17, 'צ': 18, 'ק': 19, 'ר': 20, 'ש': 21, 'ת': 22
        };

        // Function to calculate Hebrew Standard (Isopsephy) value
        function calculateHebrewStandard(input) {
            return [...input].reduce((sum, char) => sum + (hebrewStandardAlphabet[char] || 0), 0);
        }

        // Function to calculate Hebrew Ordinal value
        function calculateHebrewOrdinal(input) {
            return [...input].reduce((sum, char) => sum + (hebrewOrdinalAlphabet[char] || 0), 0);
        }

        // Function to calculate Hebrew Reduction value
        function calculateHebrewReduction(input) {
            return [...input].reduce((sum, char) => {
                let value = hebrewOrdinalAlphabet[char] || 0;
                return sum + (value ? (value > 9 ? value % 9 || 9 : value) : 0);
            }, 0);
        }

        // Main function to calculate all values
        function calculateHebrewGematria(word) {
            const standard = calculateHebrewStandard(word);
            const ordinal = calculateHebrewOrdinal(word);
            const reduction = calculateHebrewReduction(word);
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('#EntryField').on('keyup', function () {
                ...
                let data = calculateHebrewGematria(val);
